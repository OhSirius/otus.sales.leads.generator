      ...
      lead <- dc
        .run(
          leadRecordSchema
            ...
            .map({ case (lead, person) => (lead.id, person.fullName, person.phone, lead.price) }))
        .map(_.collect({ case (Some(leadId), fullName, phone, price) =>
          LeadView(leadId.id, fullName, phone, price)
        }))
    } yield lead
  }
